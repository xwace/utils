...
namespace wxw_enum
{
  ...
  template <typename E>
  constexpr auto GetEnumNames(const E v, std::vector<int> &indxes, std::string &target_str)
  {
    constexpr auto entries = magic_enum::enum_entries<E>();
    std::vector<std::string_view> valid_names;

    for (auto id_name : entries)
    {
      indxes.push_back((int)id_name.first);
      valid_names.push_back(id_name.second);
      if ((int)v == (int)id_name.first)
      {
        target_str = std::string(id_name.second);
      }
    }
  ...
  std::string BoilerPlate()
  {
    ...
    #define ENUMSTR(format) std::string(enum_to_string(format)).c_str()
  ...
  template <typename E>
  void SaveEnumNames2HeadFile(E v,std::vector<std::string_view>&names,std::vector<int>&indxes)
  {
    std::string key_name;
    GetHeader<E>(key_name);
  ...
  std::string enum_name(E v)
  {
    ...
    auto names = GetEnumNames(v, indxes, target_str);
    SaveEnumNames2HeadFile(v, names, indxes);
...
#else
#define ENUMSTR(format) std::string(wxw_enum::enum_name(format)).c_str()
#endif
