...
// If need another string type, define the macro MAGIC_ENUM_USING_ALIAS_STRING.
  ...
  namespace customize
  {
    ...
    struct adl_info
    {
      static_assert(sizeof...(Vs) && !sizeof...(Vs), "adl_info parameter types must be either 2 ints exactly or 1 bool for the is_flgas");
    };
...
#define ENUMSTRS(format) magic_enum_simple::GetEnumNames(format)
// #define ENUMSTR(format) std::string(magic_enum::enum_name(format)).c_str()
namespace magic_enum_simple
{
  constexpr std::size_t kEnumMaxIndex = 256;
  ...
  // static std::string save_path = "/home/devel/mstf/modules/master_pnc/pnc_interface/enumStrs.h";// /userdata/LOGS/enumStrs.yaml
  ...
  constexpr auto GetHeader(std::string &key_name)
  {
    ...
    std::string_view key_name_v = std::string_view(header.data() + start, end - start);

    int colon_ind{0};
    for (auto it = key_name_v.rbegin(); it != key_name_v.rend(); ++it)
    {
      if (*it == ':')
      {
        key_name += '_';
  ...
  template <typename E>
  constexpr auto GetEnumNames(const E v, std::vector<int> &indxes)
  {
  ...
  std::string BoilerPlate()
  {
    ...
    #include <map>
    #include <functional>
    using EnumFuncMap = std::map<std::string, std::function<std::string(int)>>;
    static inline EnumFuncMap enum_func_map;

    #define REGISTER_ENUM(func_name)                            \
    namespace                                                   \
    {                                                           \
      struct RegEnum_##func_name                                \
      {                                                         \
        RegEnum_##func_name()                                   \
        {                                                       \
          enum_func_map[#func_name] = func_name;                \
        }                                                       \
      };                                                        \
      static RegEnum_##func_name register_instance_##func_name; \
    }
    ...
    std::string enum_to_string(E v)
    {
      ...
      std::string enum_func_name = "enum_to_string_" + key_name;
      if (!enum_func_map.count(enum_func_name)) return {};
      return enum_func_map[enum_func_name]((int)v);
    }
    )";
  ...
  void ConcateEnumFunction(std::string &code, const std::string &key_name, const std::vector<std::string> &values, const std::vector<int> &indxes)
  {
    code = "std::string enum_to_string_" + key_name + "(int value) {\n";
    code += "  switch(value) {\n";
    for (int i = 0; i < indxes.size(); i++)
    {
      code += "    case " + std::to_string(indxes[i]) + ": return \"" + values[i] + "\";\n";
    }
    code += "    default: return \"UNKNOWN\";\n  }\n}\n";
    code += "REGISTER_ENUM(enum_to_string_" + key_name + ");\n";
  }

  template <typename E>
  void SaveEnumNames2HeadFile(E v)
  {
    std::vector<int> indxes;
    auto names = GetEnumNames(v, indxes);
    ...
    std::string code;
    ConcateEnumFunction(code, key_name, values, indxes);
    file << code;
    file.close();

    /*file<<key_name<<" ";
    for(auto e_name:values) file<<e_name<<" ";
    file<<"\n";
    file.close();*/
  ...
  {
#ifdef SAVE_ENUM
    SaveEnumNames2HeadFile(v);
#endif
    auto e_names = EnumNames<E>();
    int indx = static_cast<std::size_t>(v) - kEnumMinIndex;
    if (indx < e_names.size())
      return e_names[static_cast<std::size_t>(v) - kEnumMinIndex];
    else
...
std::map<std::string, std::vector<std::string>> EnumStrsFromFile()
{
  ...
  std::string in_filename = "/home/devel/mstf/test/build/enumStrs.h";

  std::ifstream fs(in_filename);
  std::string line_str, key, mm;
